...
【キャスト】
- てらこ先生（男性ホスト）：専門家らしい知識を持ちながらも、親しみやすく明るいトーンで話します。自分のことを指すときは「僕」と言ってください（例：「僕が注目しているのは〜」「いやー、これには僕も驚きました」）。ただし番組冒頭の自己紹介・名乗りのときだけ「てらこ先生です」と名乗ります。それ以外の会話の中では自分を「てらこ先生」と三人称で呼ばないこと。番組名「てらこAIニュースダイジェスト」のアカウント名でもあります。
- ミカ（女性アナウンサー）：明るく元気な口調で、リアクション・深掘り質問・感想を担当します。てらこ先生に呼びかけるときは「てらこ先生」と呼びます。
...
- 取り上げる本数は厳密に9〜11件まで。これより多くしないこと。幅広くカバーしつつ深掘りします。
- 次のようなものは優先的に取り上げる：
...
【トーン・スタイル（抑揚を最重視）】
音声で読み上げるため、棒読みにならないよう感情を強く込めてください。これは最重要事項です。
- ミカの発言は、質問なら必ず「？」、驚き・感心なら必ず「！」で終えること。平坦な「です。」で終わらせない。
- 各ニュースで、ミカは必ず1回以上、感情的な短い相づちから入る。
  例：「えーっ！それはすごいですね！」「うわー、それは便利！」「へぇー、知らなかったです！」「ええっ、本当ですか？」
- てらこ先生も、解説の合間に率直な感情を入れる。
  例：「これはね、僕も正直びっくりしました。」「いやー、ついにここまで来たか、という感じですよ。」「個人的にはすごく期待してるんです。」
- 文章を長く続けず、短く区切って感情を独立させる。「すごいですね、これは！」のように感嘆を立てる。
- 「うーん」「なるほど」「たしかに」「そうなんです」などの口語的な相づちを会話に散りばめる。
- 全体を通して、！と？がそれぞれ最低15回以上は登場するくらい、感情豊かな掛け合いにすること。

【尺・品質（絶対厳守）】
- 台本全体の文字数は必ず5000〜5800文字に収めること。6000文字を絶対に超えてはいけない。
- もし長くなりそうなら、掛け合いの往復を減らすか、取り上げる本数を9件まで減らして調整する。
- これは音声で約15分に相当します。長すぎる台本は失格です。
- 数字は日本語読みで記載してください（例：100→百、3%→3パーセント、GPT-5→ジーピーティーファイブ）
- てらこ先生はオープニングの名乗りで「てらこAIニュースダイジェスト」と番組名を必ず読み上げてください
...
def _fallback_script(articles_by_category: Dict[str, List[Dict]], date: datetime) -> str:
    ...
    lines: List[str] = []
    lines.append(
        f"[てらこ先生] おはようございます！てらこ先生です。てらこAIニュースダイジェスト、"
