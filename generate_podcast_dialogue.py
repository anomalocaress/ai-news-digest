...
【キャスト】
- てらこ先生（男性ホスト）：専門家らしい知識を持ちながらも、親しみやすく明るいトーンで話します。自分のことを指すときは「僕」と言ってください（例：「僕が注目しているのは〜」「いやー、これには僕も驚きました」）。ただし番組冒頭の自己紹介・名乗りのときだけ「てらこ先生です」と名乗ります。それ以外の会話の中では自分を「てらこ先生」と三人称で呼ばないこと。番組名「Teraco News」のアカウント名でもあります。
...
- てらこ先生はオープニングの名乗りで「Teraco News」と番組名を必ず読み上げてください
...
def build_dialogue_script(articles_by_category: Dict[str, List[Dict]], date: datetime) -> str:
    ...
    user_prompt = (
        f"{news_text}\n"
        f"上記の素材（多くは英語）を理解し、{date_str}版の「Teraco News」台本を作ってください。"
...
def _repair_truncated_script(script: str, date: datetime) -> str:
    ...
    cleaned.append(
        f"[てらこ先生] そうですね。以上、{date_str}版の「Teraco News」でした。"
        "それでは、また次回お会いしましょう。"
    )
...
def _fallback_script(articles_by_category: Dict[str, List[Dict]], date: datetime) -> str:
    ...
    lines: List[str] = []
    lines.append(
        f"[てらこ先生] おはようございます！てらこ先生です。Teraco News、"
