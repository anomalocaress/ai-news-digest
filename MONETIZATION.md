...
### 記事の選別・日本語化を月額プラン内で動かす（推奨）

キュレーション（Claude による記事の選別・日本語化）は、
**Claude Max プランのサブスクリプション枠**で動かせます。従量課金の API は不要です。

1. 自分のPCのターミナルで `claude setup-token` を実行し、長期トークンを発行する
2. リポジトリの Settings → Secrets and variables → Actions で
   `CLAUDE_CODE_OAUTH_TOKEN` という名前で登録する

これだけです。トークンが未設定・失効している場合は、自動的に
`CLAUDE_API_KEY`（従量課金）→ キーワード分類、の順にフォールバックします。
毎日1回の実行なので、対話で使うレート枠への影響はほぼありません。

なお、ポッドキャストの台本生成は Gemini（`GEMINI_API_KEY`）を使っており、
こちらは Claude のプランとは別です（無料枠内で収まる使用量です）。
...
| **運営者情報・免責事項** | `about.html` を自動生成しています。全ページのフッターからリンクされます。ASP審査でも提示を求められることが多いので消さないでください |
| **問い合わせ先の切り分け** | 購入後のサポート・返金・解約は各サービス提供元の管轄です。`about.html` の免責事項でその旨を明示しています |
